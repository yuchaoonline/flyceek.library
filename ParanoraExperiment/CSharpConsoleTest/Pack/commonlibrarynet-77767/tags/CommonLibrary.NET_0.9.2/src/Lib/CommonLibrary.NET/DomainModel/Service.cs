    ...
    public class EntityService<T> : IEntityService<T> where T : IEntity
    {
        ...
        public virtual BoolResult<T> Get(IActionContext ctx)
        {
            ...
            catch (Exception ex)
            {
                ...
                ExceptionManager.Handle(error, ex, ctx.Errors);
        ...
        /// <returns></returns>
        public BoolResult<IList<T>> Get(int pageNumber, int pageSize, ref int totalPages, ref int totalRecords)
        {
            IList<T> items = _repository.GetByFilter(string.Empty, typeof(T).Name + "s", pageNumber, pageSize, ref totalRecords);
            totalPages = PagerHelper.CalculatePages(totalRecords, pageSize);
        ...
        /// <param name="totalRecords">Total number of records found.</param>
        /// <returns></returns>
        public BoolResult<IList<T>> GetByFilter(string filter, int pageNumber, int pageSize, ref int totalPages, ref int totalRecords)
        {
        ...
        /// <param name="totalRecords">Total number of records found.</param>
        /// <returns></returns>
        public BoolResult<IList<T>> GetByUser(string userName, int pageNumber, int pageSize, ref int totalPages, ref int totalRecords)
        {
            string filter = "CreateUser = '" + userName + "'";
            return GetByFilter(filter, pageNumber, pageSize, ref totalPages, ref totalRecords);
        }
        ...
        /// <param name="totalRecords">Total number of records found.</param>
        /// <returns></returns>
        public BoolResult<IList<T>> GetRecent(int pageNumber, int pageSize, ref int totalPages, ref int totalRecords)
        {
        ...
        protected virtual BoolResult<T> PerformAuthentication(IActionContext ctx)
        {
            ...
            bool isAuthorized = AuthService.IsUserInRoles(_settings.EditRoles);
            string message = isAuthorized ? string.Empty : "User : " + AuthService.UserName 
        ...
        protected virtual void ApplyMassageOnAuditData(IActionContext ctx, EntityAction entityAction)
        {
            ...
            if (entityAction == EntityAction.Create || (string.IsNullOrEmpty(entity.CreateUser) && entity.CreateDate == DateTime.MinValue))
            {
                ...
                entity.CreateUser = AuthService.UserName;
            }

            // Now set update fields.
            entity.UpdateDate = DateTime.Now;
            entity.UpdateUser = AuthService.UserName;
            string timeStamp = DateTime.Now.ToShortDateString() + " - " + DateTime.Now.ToLongTimeString();
            entity.UpdateComment = "updated " + timeStamp + " by " + AuthService.UserName + " : " + entity.UpdateComment;
        }        

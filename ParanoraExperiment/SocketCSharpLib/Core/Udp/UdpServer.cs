...
namespace SocketLib.Core.Udp
{
    public class UdpServer
    {
        ...
        public event EventHandler<SocketAsyncEventArgs> OnSentedData;
        ...
        public UdpServer(IPAddress ip, int acceptPort, int dataComPort, int maxNumClient)
        {
            ...
            listen = new UdpReceiveSocket(ip,listenPort);
            listen.OnDataReceived += new EventHandler<SocketAsyncEventArgs>(listen_OnDataReceived);

            communicationRec = new UdpReceiveSocket(ip,CommunicationPort);
            ...
            communicationSend.OnDataSented += new EventHandler<SocketAsyncEventArgs>(communicationSend_DataSent);
            communicationSend.Init();
        }

        public UdpServer(string ip, int acceptPort, int dataComPort, int maxNumClient):this(IPAddress.Parse(ip),acceptPort,dataComPort,maxNumClient)
        {
        ...
        {
            if (OnSentedData != null)
            {
                OnSentedData(sender, e);
            }
        }
